...
window.onload = function () {
	...
	if (hasClass(element, 'admin-panel')) {
		...
		createNewButtons(widget, widgetControlPanel);

		// Функция создания кнопок
		function createNewButtons(widget, widgetControlPanel) {
			for (let i = 0; i < widget.length; i++) {
				const newEl = document.createElement('div');
				
				newEl.innerHTML = "<div data-id='cmv" + globalPanelIdCounter + "' class='js-cssMe-vis cssMe-vis'>" + iconEye + "</div>" +
					"<div data-id='" + widget[i].getAttribute("data-id") + "' id='cmv" + globalPanelIdCounter + "' class='cssMe__vis-box cssMe__arrow-box js-cssMe-vis-box hide'>" +
		...
		initShowAllBntsPanel(document.getElementsByClassName("js-cssMe-vis"));

		// Перебираем все панели созданны после загрузки страницы
		function initShowAllBntsPanel(jVis) {
			for (let i = 0; i < jVis.length; i++) {
				initShowBntPanel(jVis[i]);
				initHideBtnPanel(vBox[i]);
			}
		}

		// Показываем выбранную панель панель выйдя за границы контейнера
		function initShowBntPanel(jVis) {
			console.log(jVis);
			jVis.onclick = function () {
				...
				if (controllerPIC) {
					console.log($(jVis.getAttribute("data-id")));
					$(jVis.getAttribute("data-id")).classList.remove("hide"); // Усешный кейс, показываем панель с кнопками видимости
		...
		// Прячем показанную панель выйдя за границы контейнера
		function initHideBtnPanel(vBox) {
		...
		// Слушаем событие добавления нового виджета
		new CustomEvent('addedNewWidget');
		document.addEventListener('addedNewWidget', function (e) { // Для каждого виджета создается свой ивент

			// Создаем кнопки
			const newWidgetControlPanel = document.getElementById('element_' + e.detail.id).getElementsByClassName("widget-control-panel");
			const newWidget = document.querySelectorAll('#element_' + e.detail.id);

			createNewButtons(newWidget, newWidgetControlPanel);
			initShowBntPanel(document.getElementById('element_' + e.detail.id).getElementsByClassName("js-cssMe-vis")[0]);
			initHideBtnPanel(document.getElementById('element_' + e.detail.id).getElementsByClassName("js-cssMe-vis-box")[0]);
			toggleBtnLogic(document.getElementById('element_' + e.detail.id).querySelectorAll('[class*="js-cssMe__vis-"]'));
		}, false);
